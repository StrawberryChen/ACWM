...
划分训练/验证集、写入 Google Drive，并自动生成 `configs/colab.yaml`。转换后的 NPZ
会保存 `frames`、`actions` 和 LeWorld-style planning 所需的 simulator `states`：
...
如果你之前准备过旧版数据，也需要重新运行 `scripts/prepare_pusht.py`，否则 planning eval
